...
check(deep.includes('CompiledPatterns') && deep.includes('.vmat_c') && deep.includes('.vmdl_c'), 'Compiled map materials and models must remain indexed from loose files too.');
...
check(deep.includes('deepItems.TryGetValue(pair.Key'), 'Deep cache serialization must use O(1) asset lookup instead of rescanning full asset lists per entry.');
check(deep.includes('item.source.Contains(word'), 'Asset search must support map/workshop package names as search terms.');
check(deep.includes('new GameFileLoader(package, location.anchorPath)') && deep.includes('new GameFileLoader(null, location.anchorPath)'), 'Map previews must use ValveResourceFormat for both VPK and loose compiled sources.');
...
if (failures.length) {
  ...
  console.log('Deep asset self-test passed. Official-map, addon, workshop VPK and loose compiled map resources are indexed without duplicating base assets.');
}
