...
(() => {
  ...
  function errorVisual(viewport, object, reason) {
    ...
    const resource = `${object?.className || object?.name || 'entity'}|${reason || ''}`;
    if (!missing.has(resource)) { missing.add(resource); report('warning', `Showing ERROR helper for ${object?.className || object?.name || 'entity'}: ${reason || 'no Hammer visual metadata'}`); }
  ...
  function modelVisual(viewport, object, resource) {
    ...
    const fallback = errorVisual(viewport, object, `model pending/missing: ${resource}`);
    if (fallback) group.add(fallback);
    viewport.loadModel(resource).then(data => {
      if (!data?.scene) return report('warning', `Hammer editor model could not be loaded: ${resource}`, { className: object?.className });
      ...
      } catch (error) { report('error', `Could not display Hammer editor model ${resource}`, error); }
  ...
  function spriteVisual(viewport, object, hint) {
    const THREE = window.EPH_THREE || window.THREE;
    const group = new THREE.Group(); group.userData.ephVisual = true;
    const fallback = errorVisual(viewport, object, `sprite pending/missing: ${hint.resource}`); if (fallback) group.add(fallback);
    viewport.loadMaterialTexture(hint.resource).then(texture => {
      if (!texture) return report('warning', `Hammer editor sprite could not be loaded: ${hint.resource}`, { className: object?.className });
      ...
      const bounds = hint.bounds; const width = bounds ? Math.max(12, Math.abs(bounds.max[0] - bounds.min[0])) : 32; const height = bounds ? Math.max(12, Math.abs(bounds.max[2] - bounds.min[2])) : 32;
      sprite.scale.set(width, height, 1); sprite.renderOrder = 900;
      if (fallback) { group.remove(fallback); viewport.disposeObject?.(fallback); }
      group.add(sprite);
  ...
  function wireBox(object, hint) {
    ...
    visual.position.set(...center); visual.renderOrder=800; visual.userData.ephVisual=true; return visual;
  }

  function helperNumber(object, token, fallback) {
    const direct = Number(token); if (Number.isFinite(direct)) return Math.abs(direct);
    const value = Number(object?.entityProperties?.[String(token || '').trim()]); return Number.isFinite(value) ? Math.abs(value) : fallback;
  }

  function proceduralHelper(object, hint) {
    const THREE = window.EPH_THREE || window.THREE;
    const type=String(hint?.type||'').toLowerCase(), tokens=String(hint?.args||'').replace(/[{},]/g,' ').trim().split(/\s+/).filter(Boolean), color=colorFor(THREE,hint);
    if (type === 'sphere') {
      const radius=Math.max(4,helperNumber(object,tokens[0],24)); const mesh=new THREE.Mesh(new THREE.SphereGeometry(radius,16,10),new THREE.MeshBasicMaterial({color,wireframe:true,transparent:true,opacity:.8,depthTest:false})); mesh.userData.ephVisual=true; return mesh;
    }
    if (type === 'cylinder') {
      const radius=Math.max(4,helperNumber(object,tokens[0],16)), height=Math.max(8,helperNumber(object,tokens[1],32)); const edges=new THREE.EdgesGeometry(new THREE.CylinderGeometry(radius,radius,height,16)); const line=new THREE.LineSegments(edges,new THREE.LineBasicMaterial({color,depthTest:false})); line.rotation.x=Math.PI/2; line.userData.ephVisual=true; return line;
    }
    if (['line','path'].includes(type)) {
      const raw=object?.entityProperties?.[tokens[0]] || object?.entityProperties?.attachpoint || '0 0 32'; const nums=String(raw).split(/[ ,]+/).map(Number); const end=new THREE.Vector3(Number(nums[0])||0,Number(nums[1])||0,Number(nums[2])||32); const geo=new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(0,0,0),end]); const line=new THREE.Line(geo,new THREE.LineBasicMaterial({color,depthTest:false})); line.userData.ephVisual=true; return line;
    }
    if (['decal','quadbounds'].includes(type)) {
      const edges=new THREE.EdgesGeometry(new THREE.PlaneGeometry(32,32)); const line=new THREE.LineSegments(edges,new THREE.LineBasicMaterial({color,depthTest:false})); line.userData.ephVisual=true; return line;
    }
    if (['light','frustum'].includes(type)) {
      const group=new THREE.Group(); const marker=new THREE.Mesh(new THREE.SphereGeometry(6,10,8),new THREE.MeshBasicMaterial({color})); const geo=new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(0,0,0),new THREE.Vector3(0,0,-48)]); group.add(marker,new THREE.Line(geo,new THREE.LineBasicMaterial({color,depthTest:false}))); group.userData.ephVisual=true; return group;
    }
    return null;
  }
  ...
  function installMarker() {
    ...
    viewport.createEntityMarker = function(object) {
      ...
      const sprite = hints.find(h => ['iconsprite','sprite'].includes(String(h?.type||'').toLowerCase()) && h.resource);
      if (sprite) return spriteVisual(this, object, sprite);
      const bounds = hints.find(h => h?.bounds || ['bbox','wirebox'].includes(String(h?.type||'').toLowerCase()));
      if (bounds) return wireBox(object, bounds);
      for (const hint of hints) { const visual=proceduralHelper(object,hint); if (visual) return visual; }
  ...
  async function start() {
    await hydrate();
    if (!installMarker()) {
      setTimeout(installMarker,250);
      setTimeout(installMarker,700);
      setTimeout(installMarker,1500);
      setTimeout(installMarker,3000);
      setTimeout(installMarker,5000);
    }
  }
